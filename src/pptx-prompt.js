(function () {
  ...
  root.innerHTML = `
    <header class="pp-header"><div><span class="pp-eyebrow">STYLE TO POWERPOINT</span><h2>스타일을 골라, 발표자료로.</h2><p>마음에 드는 디자인을 고르면 편집 가능한 PPTX 제작용 프롬프트가 완성됩니다.</p></div><span class="pp-file-badge">.PPTX<span>제작 요청용</span></span></header>
    <div class="pp-mobile-switch" role="group" aria-label="PPTX 프롬프트 작업 화면"><button type="button" data-pp-view="gallery" aria-pressed="true">1 · 스타일 선택</button><button type="button" data-pp-view="result" aria-pressed="false">2 · 제작·복사</button></div>
    <div class="pp-workspace-grid">
      ...
      <section class="pp-result-stack" aria-label="PPTX 제작 조건과 프롬프트">
        <div class="pp-result-content">
          ...
          <div class="pp-output-section" id="ppOutputSection"><div class="pp-section-title pp-compact"><h3>PPTX 제작 프롬프트</h3><span id="ppPromptCount" class="pp-help"></span></div><p class="pp-help">복사한 프롬프트와 원본 자료를 PPTX 파일을 만들 수 있는 AI에 전달하세요.</p><p class="pp-error" id="ppError" role="alert" hidden></p><textarea id="ppOutput" class="pp-output" rows="12" readonly aria-label="PPTX 제작용 프롬프트" spellcheck="false"></textarea><div class="pp-inline-actions"><button type="button" id="pptxPromptGenerateBtn" class="pp-button">프롬프트 보기</button><button type="button" id="pptxPromptCopyBtn" class="pp-button pp-primary">프롬프트 복사</button><button type="button" id="pptxPromptDownloadBtn" class="pp-button">TXT 저장</button></div></div>
  ...
  async function copyPrompt() {
    ...
    try { await navigator.clipboard.writeText(result.prompt); status("PPTX 제작 프롬프트를 복사했습니다. 원본 자료와 함께 AI에 전달하세요."); }
    catch (_) { showPrompt(); q("#ppOutput").select(); status("프롬프트를 선택했습니다. 복사 메뉴 또는 Ctrl+C로 복사해 주세요."); }
  }
  function downloadPrompt() {
    ...
    status("PPTX 제작용 프롬프트를 TXT로 저장했습니다.");
  }
  function reset() {
