function getAllowedOptionsForPageType(key, pageType) {
  const pageOptions = (OPTIONS[key] || []).filter((item) => !Array.isArray(item.pageTypes) || item.pageTypes.includes(pageType));
...
function normalizePromptConfigSnapshot(config = {}) {
  return {
    ...
    customPhotoSubject: typeof config.customPhotoSubject === "string"
      ? config.customPhotoSubject
...
function serializePromptConfigSnapshot(config = {}) {
  ...
  return {
    ...
    customPhotoSubject: normalized.customPhotoSubject,
...
function applyPromptConfigSnapshot(config) {
  ...
  state.customPhotoSubject = next.customPhotoSubject;
...
function withPromptConfig(config, userInput, runner) {
  ...
  try {
    ...
    state.customPhotoSubject = backup.customPhotoSubject;
...
function diffPromptConfig(baseConfig = {}, targetConfig = {}, lang = "ko") {
  ...
  const changes = [];
  const pushChange = (key, label, from, to, group = "config", rawFrom = from, rawTo = to) => {
    if (JSON.stringify(rawFrom) === JSON.stringify(rawTo)) return;
    changes.push({
      key,
      label,
      group,
  ...
  pushChange(
    "customPhotoSubject",
    lang === "ko" ? "실사 소재 직접 입력" : "Custom Photo Subject",
    base.customPhotoSubject || (lang === "ko" ? "없음" : "None"),
    target.customPhotoSubject || (lang === "ko" ? "없음" : "None")
  );
  pushChange(
    "promptLineOverrides",
    ...
    "config",
    base.promptLineOverrides,
    target.promptLineOverrides
  );
...
function getPromptEditorEntries(config) {
  return withPromptConfig(config, null, () => getEditablePromptEntries().map((entry) => ({
    id: entry.id,
    sourceKey: entry.sourceKey,
    sourceLabel: entry.sourceLabel,
    sectionLabelKo: entry.sectionLabelKo,
    sectionLabelEn: entry.sectionLabelEn,
    ko: entry.ko,
    en: entry.en,
  })));
}
