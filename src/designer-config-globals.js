const PAGE_TYPES = [
  ...
  },
  {
    id: "agenda",
    text: "목차/발표 개요",
    desc: "범위·위계·일정·현재 위치를 빠르게 안내하는 페이지",
...
const OPTIONS = {
  ...
  content: [
    ...
    { icon: "IDX", text: "대형 번호·타이포 인덱스", sub: "짧은 항목을 번호와 타이포 위계로 빠르게 탐색", en: "agenda layout: large-number typographic index — short independent topics organized by scale, alignment, and whitespace without unnecessary connectors", pageTypes: ["agenda"] },
    { icon: "COL", text: "다중 열 목차", sub: "병렬 주제를 2~4개 열로 균형 있게 구분", en: "agenda layout: multi-column index — parallel topics grouped into two to four readable columns with concise supporting roles", pageTypes: ["agenda"] },
    { icon: "HIE", text: "그룹·계층 목차", sub: "상위 파트와 하위 주제를 밴드·클러스터로 구분", en: "agenda layout: grouped hierarchical index — parent chapters and child topics organized as bands or clusters with explicit hierarchy", pageTypes: ["agenda"] },
    { icon: "OWN", text: "시간·담당 목차", sub: "시간·주제·발표자 또는 담당을 표 구조로 안내", en: "agenda layout: schedule or ownership agenda — a concise two- or three-column structure for time, topic, speaker, or owner", pageTypes: ["agenda"] },
    { icon: "SPL", text: "텍스트·비주얼 분할 목차", sub: "목차와 의미 있는 사진·지도·일러스트를 분할 결합", en: "agenda layout: editorial text–visual split — a readable agenda occupying 50–60% of the canvas paired with one context-setting visual", pageTypes: ["agenda"] },
    { icon: "NAV", text: "진행 강조 내비게이터", sub: "전체 구조를 유지하고 현재 파트를 선명하게 강조", en: "agenda layout: progressive section navigator — preserve the complete agenda while highlighting the current section and muting the others", pageTypes: ["agenda"] },
    { icon: "MTX", text: "매트릭스·모자이크 목차", sub: "병렬 테마나 분류 항목을 구역으로 탐색", en: "agenda layout: matrix or mosaic index — parallel themes or categories arranged in a structured two-dimensional field without forced sequence", pageTypes: ["agenda"] },
  ],
  "text-policy": [
...
const CONTENT_FILTERS = {
  ...
  agenda: ["대형 번호·타이포 인덱스", "다중 열 목차", "그룹·계층 목차", "시간·담당 목차", "텍스트·비주얼 분할 목차", "진행 강조 내비게이터", "매트릭스·모자이크 목차", "텍스트 목록", "스토리 흐름도", "개념도/다이어그램", "타임라인", "로드맵", "프로세스 단계도"],
...
const PAGE_INSTRUCTION_PREFIX = {
  ...
  agenda: {
    ko: "아래 명세에 따라 발표자료의 목차·발표 개요 슬라이드 이미지 1장을 생성합니다. 청중이 먼저 알아야 할 범위·위계·시간·담당·현재 위치·전개 관계 중 핵심 정보를 가장 빠르게 파악하도록 구성하세요.",
    en: "Generate one agenda or presentation overview slide image based on the specifications below. Optimize the overview for the primary orientation task: scope, hierarchy, schedule, ownership, current position, or a genuine sequence/dependency.",
  },
  divider: {
...
const INPUT_GUIDES = {
  cover: {
    ...
    placeholder: "예) 해솔 이차전지 특화단지 지정 전략",
    defaultText: `[필수 입력]
- 발표 제목:
- 부제 또는 핵심 메시지:
- 기관/팀명:
- 발표 일자 또는 행사명:`,
  ...
  agenda: {
    title: "목차/발표 개요 입력",
    subtitle: "세션명과 화면에 필요한 역할·시간·담당처럼 실제로 보여줄 내용만 입력합니다.",
    hint: "아래 예시는 입력 형식 안내일 뿐이며, 사용자가 직접 입력한 내용만 프롬프트에 전달됩니다.",
    placeholder: "예) PART 1 왜 지금인가, PART 2 왜 해솔인가, PART 3 무엇을 할 것인가",
    designContextPlaceholder: "예) 청중은 전체 범위를 먼저 파악해야 함. PART 1~3은 병렬 주제이고 PART 4가 핵심 결론. 그룹형 인덱스로 파트 관계와 핵심 결론을 강조.",
    defaultText: `[필수 입력]
- 슬라이드 제목:
- 세션/파트명:
- 화면에 표시할 짧은 역할(선택):
...
- 강조할 항목(선택):`,
  ...
  divider: {
    ...
    hint: "아래 예시는 입력 형식 안내일 뿐이며, 사용자가 직접 입력한 내용만 프롬프트에 전달됩니다.",
    placeholder: "예) PART 2. 왜 해솔인가? 국내 유일의 LFP+ 거점 구조를 설명합니다.",
    defaultText: `[필수 입력]
- 섹션 제목:
- 짧은 설명 1문장:
- 필요 시 보조 설명 1문장:`,
...
const AUTO_FORBIDDEN_RULES = {
  ...
  agenda: [
    {
      ko: "목차는 짧은 항목명과 필요한 보조 정보, 실제 관계가 한눈에 읽히도록 구성하고 비주얼은 구조 인식을 돕는 역할로 사용",
      en: "keep agenda labels concise, make the real relationship immediately legible, and use visual elements to support orientation",
    },
  ],
  divider: [
    {
