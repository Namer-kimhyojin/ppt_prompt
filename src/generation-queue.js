  ...
  function createGenerationQueue({ worker, onUpdate, onDone }) {
    ...
    async function run(options = {}) {
      ...
      for (const job of state.jobs) {
        if (state.stopRequested || runId !== state.runId) break;
        ...
        let done = false;
        while (!done && job.attempts <= state.maxRetries && !state.stopRequested && runId === state.runId) {
          job.attempts += 1;
          try {
        ...
        if (!state.stopRequested && runId === state.runId && state.delayMs > 0) {
