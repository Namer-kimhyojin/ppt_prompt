...
(function () {
  ...
  const INITIAL_SUBTAB_STATES = {
    Cover: JSON.parse(JSON.stringify(COVER_STATE)),
    Divider: JSON.parse(JSON.stringify(DIVIDER_STATE)),
    Background: JSON.parse(JSON.stringify(BACKGROUND_STATE)),
    Signboard: JSON.parse(JSON.stringify(SIGNBOARD_STATE)),
  };
  ...
  function compilePrompt(prefix, state) {
    ...
    let colorSystemEn = "";
    if (state.useColorSystem) {
      const docLabelKo = prefix === "Cover" ? "표지" : prefix === "Divider" ? "간지" : prefix === "Background" ? "배경" : "안내판";
      const docLabelEn = prefix === "Cover" ? "cover" : prefix === "Divider" ? "divider" : prefix === "Background" ? "background" : "signboard";
...
- 배경색 (Background ${state.backgroundColor}): ${docLabelKo} 전체 슬라이드 캔버스 배경에 사용 — 다른 요소가 이 색 위를 덮어도 됨
...
- Background ${state.backgroundColor}: use as the full canvas background of the ${docLabelEn} — other elements may layer on top
...
- Wisely distribute the visual elements and whitespace so that decorative details never obscure the core information.`;
    ...
    const mixerBlockKo = (() => {
      ...
      if (!hasStructured) {
        return `\n[비주얼 조합 연동 — 비주얼 스타일 지시]\n- 연동 스타일명: ${ov.nameKo}\n- 비주얼 렌더링 지시문: ${ov.prompt}\n`;
      }
      if (prefix === "Cover") {
        return `\n[비주얼 조합 연동 — 비주얼 스타일 지시 (표지 전체 적용)]\n` +
          `- 연동 스타일: ${ov.nameKo}\n` +
        ...
        return `\n[비주얼 조합 연동 — 비주얼 스타일 지시 (간지 중간 강도)]\n` +
          `- 연동 스타일: ${ov.nameKo}\n` +
          (ov.textureInfo ? `- 텍스처 & 질감: ${ov.textureInfo}\n` : "") +
          (ov.colorRoles ? `- 팔레트 역할 분배: ${ov.colorRoles}\n` : "") +
          (ov.typographyGuidance ? `- 타이포그래피 처리: ${ov.typographyGuidance}\n` : "");
      } else if (prefix === "Signboard") {
        return `\n[비주얼 조합 연동 — 안내판 스타일 힌트 (시인성 우선)]\n` +
          `- 스타일 힌트: ${ov.nameKo}\n` +
          (ov.textureInfo ? `- 질감 요소 (낮은 대비, 정보 영역 방해 금지): ${ov.textureInfo}\n` : "") +
          (ov.colorRoles ? `- 팔레트 역할 분배: ${ov.colorRoles}\n` : "");
      } else {
        return `\n[비주얼 조합 연동 — 배경 스타일 힌트 (절제 적용)]\n` +
          `- 스타일 힌트: ${ov.nameKo}\n` +
    ...
    const mixerBlockEn = (() => {
      ...
      if (prefix === "Cover") {
        ...
        return `\n[Concept Mixer — Signboard Style Hint (prioritize visibility)]\n` +
          `- Style hint: ${ov.nameEn}\n` +
          (ov.textureInfo ? `- Texture only (low contrast; never obstruct information): ${ov.textureInfo}\n` : "") +
          (ov.colorRoles ? `- Palette role distribution: ${ov.colorRoles}\n` : "");
      } else {
    ...
    if (state.outputLang === "ko") {
      const docTypeLabel = prefix === "Cover" ? "표지" : prefix === "Divider" ? "간지" : prefix === "Signboard" ? "행사 안내판" : "배경";
      const requestAssetLabel = prefix === "Signboard" ? docTypeLabel : `PPT 문서 ${docTypeLabel}`;
      const specificationLabel = prefix === "Signboard" ? docTypeLabel : `문서 ${docTypeLabel}`;
      const targetAssetLabel = prefix === "Cover"
        ? "타이틀 표지 슬라이드 (Title Cover)"
        : prefix === "Divider"
          ? "구역 분리 간지 슬라이드 (Section Divider)"
          : prefix === "Signboard"
            ? "행사 정보 안내판 (Event Information Signboard)"
            : "본문 내지 배경 템플릿 (Slide Background)";
      ...
      return `[작업 요청] 다음 상세 요구사항과 디자인 가이드라인을 엄격히 준수하여 전문 디자인 에이전시 수준의 고품질 ${requestAssetLabel} 이미지를 생성해 주세요.

[${specificationLabel} 디자인 사양]
규격 및 방향: ${layoutRatio}, ${orientationTextKo}
대상 슬라이드 유형: ${targetAssetLabel}
${conflictBlock}
[디자인 연출 및 레이아웃 구조]
${layoutLinesKo.join("\n")}
...
- ⛔ 텍스트 절대 금지 사항: 지정된 텍스트를 영어로 번역하거나, 의역하거나, 요약하거나, 임의로 수정하는 행위는 절대 허용되지 않습니다. 입력된 원문 텍스트를 한 글자도 바꾸지 말고 그대로 렌더링하십시오. 또한, 명시적으로 지정된 텍스트 외에 임의의 더미 텍스트, 가짜 단어, 라벨, 가짜 글자, 알파벳, 숫자, 기호를 이미지 내에 생성하거나 그려 넣지 마십시오. 지정되지 않은 공간은 완전한 빈 여백으로 유지해야 합니다.`;
      ...
      const docTypeLabelEn = prefix === "Cover" ? "Cover" : prefix === "Divider" ? "Divider" : prefix === "Signboard" ? "Information Signboard" : "Background";
      const requestAssetLabelEn = prefix === "Signboard" ? "information signboard" : `PPT document ${docTypeLabelEn}`;
      const specificationLabelEn = prefix === "Signboard" ? "Information Signboard" : `Document ${docTypeLabelEn} Page`;
      const targetAssetLabelEn = prefix === "Cover"
        ? "Title Cover Slide"
        : prefix === "Divider"
          ? "Chapter/Section Divider Slide"
          : prefix === "Signboard"
            ? "Event Information Signboard"
            : "Content Background Template Slide";

      return `[TASK REQUEST] Please generate a high-quality premium ${requestAssetLabelEn} design image that strictly adheres to the following detailed requirements and professional design agency guidelines.

[${specificationLabelEn} Design Specifications]
Standard & Direction: ${layoutRatio}, ${orientationTextEn}
Asset Style Target: ${targetAssetLabelEn}
${conflictBlockEn}
...
- ⛔ ABSOLUTE TEXT RULE: You are strictly prohibited from translating, paraphrasing, summarizing, or altering any of the specified text strings in any way. Render every quoted text string exactly as-is — do not change even a single character. Additionally, you are strictly prohibited from generating, adding, or overlaying any arbitrary text, dummy words, gibberish characters, fake labels, placeholder symbols, or brand names on the image. Leave all other regions completely clean and blank as empty whitespace.`;
  ...
  function resetData(prefix, state) {
    const initialState = INITIAL_SUBTAB_STATES[prefix];
  ...
  function _showSlideDocMixerBadge(nameKo) {
    ...
    badge.innerHTML = `<span class="slide-doc-mixer-badge-icon">🎨</span> 비주얼 조합 연동 중: <strong>${nameKo}</strong> <button type="button" class="slide-doc-mixer-badge-clear" id="btnSlideDocMixerClear">연동 해제</button>`;
  ...
  window.applyMixerToSlideDocument = function(mixerData) {
    ...
    setupColorPresets('slideBackgroundColorPresetGrid', 'background');
    setupColorPresets('slideSignboardColorPresetGrid', 'signboard');
    _showSlideDocMixerBadge(mixerData.nameKo || '비주얼 조합');
