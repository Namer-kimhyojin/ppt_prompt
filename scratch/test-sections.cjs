...
`,
  ...
  const helpers = {
    isAiColorStrategy: () => false,
    isConceptGeneratedPromptValue: () => true,
    getNonConceptPromptLines: () => [],
    shouldUseCompactPromptGuidance: () => false,
    isBasicVisualPlanningMode: () => true,
    isDetailVisualPlanningMode: () => false,
    hasBasicConceptPromptInput: () => true,
    conceptStripValuesFromState: () => [],
    visualPlanningModeLabel: () => "기본 모드",
    compactConceptSummary: (v) => v,
    kindBadgeHtml: () => "",
    status: () => {},
    localizeValue: (v) => v,
    localizeHeading: (ko, en) => lang === "en" ? en : ko,
    localizeSentence: (ko, en) => lang === "en" ? en : (lang === "bilingual" ? `${ko} / ${en}` : ko),
    getDefaultQualityTagLines: () => [],
    getEffectiveOrientation: () => "vertical",
    getPromptSpecificationSummary: () => "1:1",
    visibleTextEntries: () => [
      { key: "headline", label: "헤드라인", value: "배터리 교육원 모집" },
      { key: "bodyCopy", label: "본문", value: "모집 요강 설명" }
    ],
    instructionEntries: () => [],
    backgroundModeLabel: () => "단색",
  };
  
  PROMO_PROMPT.init(testState, helpers);
