...
try {
  ...
  result = await json("/api/admin-settings");
  assert.equal(Object.hasOwn(result.body, "pollinationsPublicKey"), false);
  assert.equal(result.body.programName, "PromptDeck Pages Admin Test");
  ...
  try {
    ...
    assert.equal(await page.locator("#adminPollinationsKey").count(), 0);
    assert.equal(await page.evaluate(() => Object.hasOwn(JSON.parse(localStorage.getItem('promptdeck_admin') || '{}'), 'pollinationsPublicKey')), false);
