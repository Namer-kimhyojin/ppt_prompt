...
try {
  ...
  await guidePage.locator('[data-action="open-layout"]').click();
  await guidePage.locator('#dwLayoutGrid[aria-busy="false"].is-ready').waitFor();
  await guidePage.locator('#dwLayoutTabs [data-layout-choice="body-columns"]').click();
  await saveGuide("layouts", "1440px desktop comparison of three body arrangements with identical content, colors and paper");
  await guidePage.locator('[data-layout-action="apply"]').click();
  await guidePage.locator('[data-action="open-overview"]').click();
  await guidePage.locator('#dwOverviewGrid[aria-busy="false"]').waitFor();
  await saveGuide("overview", "Current document set with the selected two-column body arrangement");
  await guidePage.locator('#dwOverviewDialog [data-layout-close]').last().click();
  await guidePage.locator('#documentDesignApp [data-action="undo"]').click();
  await guidePage.locator('#dwDesktopControls [data-action="open-studio"]').click();
  await guidePage.waitForSelector('#dwStudioPreview[aria-busy="false"] .dd-page');
  await guidePage.locator('[data-studio-group="tableStyle"]').click();
  await guidePage.waitForSelector('#dwStudioPreview[aria-busy="false"] .dd-page');
  await guidePage.locator('[data-studio-choice="striped"]').click();
  await guidePage.waitForSelector('#dwStudioPreview[aria-busy="false"] .dd-page');
  ...
  await guidePage.locator('[aria-label="느낌 조정 닫기"]').click();
  await guidePage.locator('[data-action="open-layout"]').click();
  await guidePage.locator('#dwLayoutGrid[aria-busy="false"].is-ready').waitFor();
  await guidePage.locator('#dwLayoutTabs [data-layout-choice="body-columns"]').click();
  await saveGuide("layouts-mobile", "390px mobile layout choice showing one full document at a time");
  await guidePage.locator('#dwLayoutDialog [data-layout-close]').last().click();
  await guidePage.locator('[data-action="open-density"]').click();
  await guidePage.waitForSelector('#dwStudioPreview[aria-busy="false"] .dd-page');
  await guidePage.locator('[data-studio-choice="table"]').click();
  await guidePage.waitForSelector('#dwStudioPreview[aria-busy="false"] .dd-page');
  await guidePage.locator('[data-studio-zoom]').click();
  await saveGuide("density-mobile", "390px mobile inspection of a multi-page sample table; example density is not saved as design settings");
  await guidePage.locator('#dwStudioDialog [data-studio-close]').first().click();
  await guidePage.locator('.dw-header [data-action="open-library"]').click();
  await guidePage.locator('#dwLibraryName').fill('분기 실적 보고 · 정갈한 편집');
  await guidePage.locator('#dwLibrarySave button').click();
  await guidePage.locator('[data-library-apply]:not([disabled])').waitFor();
  await saveGuide("library-mobile", "390px mobile named design library with reusable settings import and export");
  await guidePage.locator('#dwLibraryDialog [data-library-close]').first().click();
