...
const headers = ["식별자", "성명", "소속", "앞면본문", "링크", "뒷면내용"];
const suggested = mapping.suggest(headers, { duplex: true });
assert.equal(suggested.id, "식별자");
assert.equal(suggested["data.name"], "성명");
assert.equal(suggested["data.category"], "소속");
assert.equal(suggested["front.body"], "앞면본문");
...
assert.equal(record.data.name, "김배터리");
assert.equal(record.data.category, "샘플교육센터");
...
assert.equal(manual["front.body"], "앞면본문", "remaining unclaimed columns must still be auto-matched");
