...
신청링크: https://example.kr/apply`);
    ...
    await page.evaluate(() => window.PromptDeckTabs.switchTab("qrGenerator"));
    await page.waitForSelector("#paneQrGenerator.active");
    ...
    const desktopQrLabelAction = page.locator("#tabActions [data-proxy-target='qrGeneratorPrintBtn']");
    record(await desktopQrLabelAction.isVisible(), "QR built-in label output was not restored to the desktop action dock", failures);
    record((await desktopQrLabelAction.textContent()).trim() === "QR 라벨 출력", "QR desktop label output action was not clearly named", failures);
