...
function getUserInputParts() {
  if (state.promptInputOverride && typeof state.promptInputOverride === "object") {
    return {
      content: String(state.promptInputOverride.content || "").trim(),
      designContext: String(state.promptInputOverride.designContext || "").trim(),
      exclusions: String(state.promptInputOverride.exclusions || "").trim(),
    };
  }
...
function getAllowedOptions(key) {
  const pageOptions = (OPTIONS[key] || []).filter((item) => !Array.isArray(item.pageTypes) || item.pageTypes.includes(state.pageType));
  if (key !== "content") return pageOptions;
  const allowed = CONTENT_FILTERS[state.pageType];
  if (!allowed) return pageOptions;
  const optionByText = new Map(pageOptions.map((item) => [item.text, item]));
  return allowed.map((text) => optionByText.get(text)).filter(Boolean);
}
...
function isFooterNoneSelected() {
  ...
  return Boolean(footerBar && (footerBar.icon === "NONE" || footerBar.text === "없음"));
}
...
function getAutoForbiddenRuleEntries() {
  ...
  if (!isFullDesignReference() && hasFixedColorSystem()) {
    common.push({
      ko: "팔레트 역할색을 사진·지도·재료·피부·식생·하늘 전체에 덮어 단일 색조로 만들지 말 것. 팔레트는 구조·주석·데이터·프레이밍에 적용하고, 실사의 고유색과 의미 기반 기능색은 보존하되 장식 목적의 임의 색상 추가는 금지",
      en: "DO NOT wash photography, maps, materials, skin, vegetation, or sky into one palette hue. Apply palette colors to structure, annotation, data, and framing; preserve natural local colors and meaning-led functional colors, while forbidding arbitrary decorative colors.",
    });
  }
...
function buildColorSystemText(lang) {
  if (isAiDelegatedColorSystem()) {
    return lang === "ko"
      ? [
          ...
          "▸ 팔레트는 UI 구조와 데이터 역할을 통제하는 색상 앵커이며 사진·지도·재료의 자연색을 대체하지 않는다. 전체 화면에 단일 색조 워시를 적용하지 않는다.",
          "▸ 페이지마다 실제 장면·재료·데이터 의미에서 나온 색온도 대비점 하나를 선택할 수 있다. 분석 장면은 차가운 구조색, 현장·사람·실행 장면은 자연색이나 따뜻한 재료색을 살려 덱의 긴장과 리듬을 만든다.",
          "▸ 덱 아이덴티티는 Primary의 연속 흐름선·호·방향축, Secondary의 증거면·보조 밴드, Accent의 단일 결정점으로 만든다. 동일한 색상 모티프를 페이지 목적에 맞춰 크기와 위치만 변주한다.",
        ].join("\n")
      : [
          ...
          "▸ Treat the palette as an anchor for UI structure and data roles, not as a replacement for the local colors of photography, maps, or materials. Never apply a single-hue wash across the full canvas.",
          "▸ Each slide may use one chromatic counterpoint derived from its real scene, material, or data meaning. Let analytical pages lean cooler while field, people, and action pages preserve natural or warmer material color to create deck-level tension and rhythm.",
          "▸ Build deck identity through a continuous Primary spine, arc, or directional current; Secondary evidence fields and supporting bands; and one Accent decision point. Vary only scale and placement according to the page purpose.",
        ].join("\n");
  }
  ...
  if (lang === "ko") {
    return [
      ...
      `\u25B8 \uadf8\ub798\ud53d \uc694\uc18c\ub294 \uc9c0\uc815 \ud314\ub808\ud2b8\uc758 \uc5ed\ud560 \uc0c9\uc0c1\uc744 \uc6b0\uc120\ud558\ub418, \ud770\uc0c9\u00b7\ud68c\uc0c9 \ub4f1\uc758 \uc911\uc131\uc0c9\uacfc \uc2e4\uc0ac \uc774\ubbf8\uc9c0\uc758 \uc790\uc5f0\uc2a4\ub7ec\uc6b4 \uace0\uc720 \uc0c9\uc0c1\uc740 \uc790\uc720\ub86d\uac8c \uc0ac\uc6a9\ud55c\ub2e4`,
      `\u25B8 \ud314\ub808\ud2b8 \uc801\uc6a9 \ubc94\uc704: \ud5e4\ub354\u00b7\ud504\ub808\uc784\u00b7\uc8fc\uc11d\u00b7\ub370\uc774\ud130 \uad6c\uc870. \uc0ac\uc9c4\u00b7\uc9c0\ub3c4\u00b7\uc7ac\ub8cc\u00b7\ud53c\ubd80\u00b7\uc2dd\uc0dd\u00b7\ud558\ub298\uc740 \uace0\uc720\uc0c9\uc744 \ubcf4\uc874\ud558\uace0 \uc804\uccb4 \ube14\ub8e8 \uc624\ubc84\ub808\uc774\ub098 \ub2e8\uc77c \uc0c9\uc870 \uc6cc\uc2dc\ub97c \uc0ac\uc6a9\ud558\uc9c0 \uc54a\ub294\ub2e4`,
      `\u25B8 \uc0c9\uc0c1 \ub4dc\ub77c\ub9c8\ud22c\ub974\uae30: \ud55c \ud398\uc774\uc9c0\uc5d0\ub294 \uc2e4\uc81c \uc7a5\uba74\u00b7\uc7ac\ub8cc\u00b7\ub370\uc774\ud130 \uc758\ubbf8\uc5d0\uc11c \ub098\uc628 \uc0c9\uc628\ub3c4 \ub300\ube44\uc810 \ud558\ub098\ub9cc \ucd94\uac00\ud560 \uc218 \uc788\ub2e4. \ucc28\uac00\uc6b4 \uad6c\uc870\uc0c9\uacfc \ub530\ub73b\ud55c \ud604\uc7a5\u00b7\uc778\ubb3c\u00b7\uc7ac\ub8cc\uc0c9\uc758 \ub300\ube44\ub85c \uae34\uc7a5\uacfc \uad81\uae08\uc99d\uc744 \ub9cc\ub4e4\ub418, \uc0c8\ub85c\uc6b4 \uc0ac\uc2e4\uc774\ub098 \uc7a5\uc2dd\uc0c9\uc744 \ub9cc\ub4e4\uc9c0 \uc54a\ub294\ub2e4`,
      `\u25B8 \ub370\uc774\ud130 \uacc4\uc5f4\uc740 Primary \ub2e8\uc77c\uc0c9\uc744 \ubc18\ubcf5\ud558\uc9c0 \ub9d0\uace0 Secondary \ud30c\uc0dd \uba85\ub3c4, \uc911\uc131\uc0c9, \uc758\ubbf8\uc5d0 \ub9de\ub294 \ub300\ube44\uc0c9 \ud558\ub098\ub85c \uad6c\ubd84\ud55c\ub2e4`,
      `\u25B8 \ud314\ub808\ud2b8 \uc11c\uba85: Primary ${primary}\ub294 \ud398\uc774\uc9c0\ub97c \uad00\ud1b5\ud558\uac70\ub098 \ud575\uc2ec \ub178\ub4dc\ub97c \uc787\ub294 \ud558\ub098\uc758 \uc5f0\uc18d \ud750\ub984\uc120\u00b7\ud638\u00b7\ubc29\ud5a5\ucd95, Secondary ${secondary}\ub294 \uadfc\uac70\uac00 \ub193\uc774\ub294 \uc99d\uac70\uba74\u00b7\ubcf4\uc870 \ubc34\ub4dc, Accent ${accent}\ub294 \ud398\uc774\uc9c0\ub2f9 \ucd5c\ub300 \ud55c \uacf3\uc758 \uacb0\uc815\u00b7\ubcd1\ubaa9\u00b7\uc804\ud658\uc810\uc5d0\ub9cc \uc791\uc740 \ub178\uce58\u00b7\uc810\u00b7\ud654\uc0b4\ud45c\ub85c \ubc18\ubcf5\ud55c\ub2e4`,
      `\u25B8 \uc2e4\uc0ac \uc870\ud654: \ud314\ub808\ud2b8\uc0c9\uc740 \uc0ac\uc9c4 \ud504\ub808\uc784\u00b7\ucea1\uc158\u00b7\uc5f0\uacb0\uc120\uc5d0\ub9cc \uc801\uc6a9\ud558\uace0 \uc0ac\uc9c4 \uc790\uccb4\ub294 \uc7ac\ucc29\uc0c9\ud558\uc9c0 \uc54a\ub294\ub2e4`,
    ].join("\n");
  } else {
    return [
      ...
      `\u25B8 Prioritize the defined palette roles for graphic elements, while freely using neutral colors such as white and gray and the natural inherent colors of photographic images`,
      `\u25B8 Palette scope: header, framing, annotation, and data structure. Preserve the local colors of photography, maps, materials, skin, vegetation, and sky; never apply a full-canvas blue overlay or single-hue wash`,
      `\u25B8 Color dramaturgy: allow one chromatic counterpoint per slide, derived from the actual scene, material, or data meaning. Use contrast between cool structure and warmer field, people, or material color to create tension and curiosity without inventing facts or decorative colors`,
      `\u25B8 For data series, avoid repeating Primary alone; distinguish series with Secondary tonal steps, neutrals, and one meaning-led counterpoint`,
      `\u25B8 Palette signature: repeat Primary ${primary} as one continuous spine, arc, or directional current; use Secondary ${secondary} for evidence fields and supporting bands; reserve Accent ${accent} for one small notch, dot, or arrow marking at most one decision, bottleneck, or transition per slide`,
      `\u25B8 Photography harmony: apply palette color only to frames, captions, and connectors; never recolor the photograph itself`,
    ].join("\n");
  }
}
...
function buildColorFields() {
  ...
  const updatePreview = () => {
    ...
    previewBox.innerHTML = `
      ...
      <div style="margin-top: 8px; font-size: 12px; opacity: 0.8;">Sample text and layout preview</div>
    `;
  };
...
function buildFullPrompt(lang) {
  ...
  // 공통 프롬프트는 최종 실행 명령 아래에 결합되는 첫 번째 H2 섹션이다.
  lines.push(lang === "ko" ? "## 공통 디자인 시스템" : "## COMMON DESIGN SYSTEM");
  lines.push("");
  lines.push(lang === "ko"
    ? `이 섹션은 ${pageLabel}에 공통 적용할 디자인 시스템과 제작 제약을 정의합니다. 현재 슬라이드의 실제 표시 내용은 개별 슬라이드 명세에서 가져오며, 개별 명세에 없는 디자인 결정에만 아래 공통 규칙과 기본값을 적용합니다.`
    : `This section defines the reusable design system and production constraints for a ${pageLabel}. Take the actual visible content from the individual slide specification, and apply the common rules and defaults below only where that specification does not provide a slide-specific decision.`);
  lines.push("");

  // --- 페이지별 생성 지침 ---
  if (addPreamble) {
    lines.push(lang === "ko" ? "### 생성 지침" : "### GENERATION GUIDANCE");
    lines.push("");
...
function updateUserInputGuide(forceFill = false) {
  ...
  if (designContext) {
    designContext.placeholder = guide.designContextPlaceholder
      || "발표자 스크립트, 의도, 청중에게 전달할 흐름을 입력하세요. 이 내용은 슬라이드에 원문 그대로 넣지 않습니다.";
  }
...
function copyPrompt(lang) {
  ...
  const generator = window.PromptDeckSlidePromptGenerator;
  const config = window.PromptDeck?.captureCurrentConfig?.() || {
    pageType: state.pageType,
  ...
  const promptPackage = {
    schemaVersion: 1,
    contractVersion: window.PromptDeckSkillPresetContract?.versions?.plannerCurrent || "3.6",
    designContractVersion: "legacy",
    plannerContractVersion: window.PromptDeckSkillPresetContract?.versions?.plannerCurrent || "3.6",
    skillPresetContractVersion: window.PromptDeckSkillPresetContract?.versions?.skillPresetCurrent || "1.0",
    source: "slide-designer",
    text,
    lang,
    config,
    userInput: getUserInputParts(),
    targetModel: "common",
    outputMode: config?.promptSettings?.outputMode || "block",
  };

  if (generator && typeof generator.setCommonPrompt === "function") {
    generator.setCommonPrompt(promptPackage);
  } else {
