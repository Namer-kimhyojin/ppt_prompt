const state = {
  ...
  bgSolidColor: "#F5F6F7",   // bg-style 이 '단색 배경'일 때 사용할 HEX
  promptLineOverrides: {},
  promptLineDrafts: {},
...
function hasSelection(key) {
  ...
  if (key === "color-system") {
    return (
      ...
      Object.entries(state.colorSystem).some(([name, value]) => value !== DEFAULT_COLOR_SYSTEM[name])
    );
  }
...
function formatUserInputPreview(parts, lang) {
  ...
  if (parts.exclusions) {
    if (lines.length) lines.push("");
    lines.push(lang === "ko" ? "[슬라이드에 넣지 말 내용]" : "[Content to exclude from the slide]");
...
function findOptionByText(key, text) {
  return (OPTIONS[key] || []).find((item) => item.text === text) || null;
}
...
const FULL_REFERENCE_OMITTED_KEYS = [
  "mood", "quality", "material", "lighting",
...
function isOptionDisabled(key) {
  ...
  // 실사 합성이 미선택이거나 '사용 안 함'만 선택되어 있으면 피사체 선택을 비활성화
  ...
  // 배경 톤은 bg-style이 '단색 배경' 또는 '장식 없음'일 때 의미 없음
  if (key === "bg-tone") {
    const bgStyle = state.selections["bg-style"];
    if (!bgStyle) return true;
    if (bgStyle.text === "단색 배경" || bgStyle.text === "장식 없음 (기본)") return true;
  }
...
function getDisableReason(key) {
  ...
  if (key === "photo-subject") {
    const texts = getSelectionValues("photo-composite").map((v) => v.text);
    if (!texts.length) return "먼저 위에서 '실사 합성 방식'을 1개 이상 선택하면 활성화됩니다.";
    if (texts.every((t) => t === "사용 안 함")) return "'실사 합성 방식'이 사용 안 함으로 설정되어 있습니다.";
  }

  if (key === "bg-tone") {
    const bgStyle = state.selections["bg-style"];
    if (!bgStyle) return "먼저 '배경 콘텐츠 종류'를 선택하면 활성화됩니다.";
    if (bgStyle.text === "단색 배경") return "단색 배경은 컬러/흑백 구분이 의미 없습니다. 아래 단색 컬러 입력으로 직접 지정하세요.";
...
function buildColorSystemText(lang) {
  ...
  const solidBgActive = state.selections["bg-style"]?.text === "단색 배경";
...
function getSelectionCanonicalId(key, item) {
  ...
  if (key === "bg-style" && item?.icon === "SOL") return "background-surface";
    ...
    section.keys.forEach((key) => {
      ...
      if (key === "header-bar-settings") {
        ...
        entries.push(makePromptEntry(
          ...
          headerEnabled
            ? `\u25B8 \uc0c1\ub2e8\ubc14 \uc0ac\uc6a9  |  \ub192\uc774: ${headerHeight}px  |  \uceec\ub7ec: ${headerColor.toUpperCase()}`
            : `\u25B8 \uc0c1\ub2e8\ubc14 \uc5c6\uc74c \u2014 \uc2ac\ub77c\uc774\ub4dc \uc0c1\ub2e8\uc5d0 \ubcc4\ub3c4 \ubc14 \uad6c\uc870\ub97c \uc0ac\uc6a9\ud558\uc9c0 \uc54a\uc74c`,
          headerEnabled
            ? `\u25B8 Header bar: enabled  |  height: ${headerHeight}px  |  color: ${headerColor.toUpperCase()}`
      ...
      if (key === "footer-bar-settings") {
        ...
        entries.push(makePromptEntry(
          ...
          footerEnabled
            ? `\u25B8 \ud558\ub2e8\ubc14 \uc0ac\uc6a9  |  \ub192\uc774: ${footerHeight}px  |  \uceec\ub7ec: ${footerColor.toUpperCase()}`
            : `\u25B8 \ud558\ub2e8\ubc14 \uc5c6\uc74c \u2014 \uc2ac\ub77c\uc774\ub4dc \ud558\ub2e8\uc5d0 \ubcc4\ub3c4 \ubc14 \uad6c\uc870\ub97c \uc0ac\uc6a9\ud558\uc9c0 \uc54a\uc74c`,
          footerEnabled
            ? `\u25B8 Footer bar: enabled  |  height: ${footerHeight}px  |  color: ${footerColor.toUpperCase()}`
...
function buildBgSolidColorField() {
  ...
  wrap.hidden = state.selections["bg-style"]?.text !== "단색 배경";
...
function selectOption(key, optionText) {
  ...
  if (key === "bg-style") {
    ...
    } else if (option.text === "단색 배경" || option.text === "장식 없음 (기본)") {
      state.selections["bg-tone"] = null;
    }
  }

  // 실사 합성: '사용 안 함'은 다른 방식과 상호 배타. 피사체도 자동 해제
...
function updateClearButtonStates() {
  Object.keys(OPTION_META).forEach((key) => {
    ...
    if (!button) return;
    const disabled = key !== "color-system" && isOptionDisabled(key);
...
function renderOptionTree() {
  ...
  SECTION_DEFS.forEach((section) => {
    ...
    button.addEventListener("click", () => focusSection(section.id, section.groups[0]));
...
function focusGroup(key) {
  state.focusKey = key;
  const target = key === "user-content" ? document.getElementById("group-user-content") : document.getElementById(`group-${key}`);
  if (!target) return;
...
function updateTreeState() {
  ...
  document.querySelectorAll(".tree-leaf").forEach((leaf) => {
    ...
    leaf.title = customized ? "직접 수정된 프롬프트가 있습니다." : "";
...
function buildPromptPartsLegacy(lang) {
  ...
  Object.entries(SECTION_MAP).forEach(([sectionId, section]) => {
    ...
    section.keys.forEach((key) => {
      ...
      if (key === "bg-solid-color") {
        // bg-style이 '단색 배경'일 때만 HEX를 함께 출력
        const bgStyle = state.selections["bg-style"];
        if (bgStyle && bgStyle.text === "단색 배경") {
          const hex = state.bgSolidColor.toUpperCase();
          lines.push(lang === "ko"
      ...
      getSelectionValues(key).forEach((item) => {
        if (key === "format" && item.text === "사용자 정의") {
          ...
          lines.push(lang === "ko" ? item.text : item.en);
        }
      });
    });
...
function renderPrompt() {
  ...
  if (conflicts.length) {
    ...
    conflictTitle.textContent = `⚠ 상충 조합 ${conflicts.length}건 감지됨`;
    conflictBox.appendChild(conflictTitle);

    conflicts.forEach((c) => {
      const item = document.createElement("div");
      item.className = "conflict-warning-item";
      const message = state.lang === "ko" ? c.msg : c.msgEn;
      item.textContent = `${message} ${getConflictResolutionText(c, state.lang)}`;
...
function buildProsePrompt(lang) {
  ...
  if (userInput.exclusions) {
    sentences.push(lang === "ko" ? `[슬라이드에 넣지 말 내용] ${userInput.exclusions}` : `[Content to exclude from the slide] ${userInput.exclusions}`);
  }
...
function buildFullPrompt(lang) {
  ...
  if (outputMode === "prose") {
    ...
    if (userInput.exclusions) {
      lines.push(lang === "ko" ? "## 슬라이드에 넣지 말 내용" : "## Content To Exclude");
      lines.push("");
      lines.push("```");
