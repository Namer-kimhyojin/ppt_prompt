...
이 저장소는 정적 HTML/CSS/JavaScript 기반의 탭형 프롬프트 생성 서비스입니다. 주요 기능은 슬라이드 프롬프트 설계, 슬라이드 분리, API 이미지 생성, 지도 이미지 프롬프트, 부속 양식 생성, 컨셉 제안, 비주얼 믹서, 양식 이미지 생성, 홍보용 이미지 생성, QR코드 생성입니다.
...
- `src/slide-document.js`: 부속 양식 탭의 문서 표지/간지/배경/안내판 생성.
- `src/concept-suggest.js`: 컨셉 제안 탭.
- `src/concept-mixer.js`: 비주얼 믹서 탭.
- `src/form-image.js`: 양식 이미지 탭.
- `src/promotion.js`: 홍보용 이미지 생성.
- `src/qr-generator.js`: QR코드 생성기.
...
- `styles/concept-suggest.css`: 컨셉 제안.
- `styles/form-image.css`: 양식 이미지.
- `styles/promotion.css`: 홍보용 이미지.
- `styles/qr-generator.css`: QR코드 생성기.
...
6. `src/tabs.js`의 `actionSets`와 `actionHost` 등록
   우측 패널 상단 액션 도크에 표시할 핵심 액션과 도크가 들어갈 패널을 지정한다.
...
- `tabBtnConceptMixer` ↔ `paneConceptMixer`
- `tabBtnFormImage` ↔ `paneFormImage`
- `tabBtnPromotion` ↔ `panePromotion`
- `tabBtnQrGenerator` ↔ `paneQrGenerator`

## 퀵버튼/탭 바 레이아웃 규칙

- `.app-tabs-bar`는 탭 탐색 전용으로 사용하고 `#tabActions`를 넣지 않는다.
- 데스크톱에서는 `#tabActions`를 활성 탭의 `actionHost` 우측 패널 최상단에 sticky 액션 도크로 배치한다.
- 직접 노출하는 핵심 액션은 최대 3개를 권장하고 나머지는 `더보기` 메뉴로 묶는다.
- 모바일에서는 우측 패널 액션 도크를 숨기고, 기존 모바일 하단 액션 또는 본문 내 액션을 사용한다.
...
- 서브탭은 `문서 표지 생성`, `문서 간지 생성`, `문서 배경 생성`, `안내판 생성` 네 가지가 유지되어야 한다.
- 상단 퀵버튼은 `slideDocCopyPromptBtn`, `slideDocSampleBtn`, `slideDocResetBtn`를 프록시한다.
- 표지/간지/배경/안내판 중 하나를 수정할 때는 공통 로직이 다른 유형에 영향을 주는지 확인한다.
...
- 부속 양식 탭 버튼 표시
- 표지/간지/배경/안내판 서브탭 표시
- `#tabActions`가 활성 탭의 우측 `actionHost` 최상단에 있는지
- 탭 바에 세로 스크롤바가 없는지
...
- 데스크톱: 탭 바는 탐색만 표시하고 퀵버튼은 활성 탭의 우측 패널 최상단에 고정.
- 모바일: 탭은 깨지지 않고 표시, 우측 패널 액션 도크는 숨김, 하단 또는 본문 액션 사용.
...
- 탭별 핵심 액션은 우측 패널 상단 액션 도크에 연결한다.
- 버튼 텍스트는 짧고 명확하게 쓴다.
